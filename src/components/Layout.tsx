  ...
  return (
    <div className="min-h-screen flex flex-col w-full bg-gradient-to-br from-background to-muted/50">
      ...
      <div className="flex flex-1 w-full">
        <AppSidebar />
        <main className={`flex-1 p-4 md:p-6 relative ${isReportPage ? 'md:p-4' : ''}`}>
          ...
          <div className={`pt-8 md:pt-0 max-w-7xl mx-auto w-full ${isReportPage ? 'h-full' : ''}`}>
            <div className={`${!isReportPage ? 'bg-white/80 backdrop-blur-sm rounded-xl shadow-sm p-4 md:p-6 border border-gray-100 transition-all duration-300 hover:shadow-md' : ''}`}>
              <Outlet />
            </div>
          </div>
        </main>
